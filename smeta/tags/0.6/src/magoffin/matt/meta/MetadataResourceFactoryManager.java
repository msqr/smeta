/* ===================================================================
 * MetadataResourceFactoryManager.java
 * 
 * Created Jan 15, 2007 9:50:37 PM
 * 
...
public class MetadataResourceFactoryManager {
	...
	private Map<String, MetadataResourceFactory> factoryCache 
		= Collections.synchronizedMap(
				new WeakHashMap<String, MetadataResourceFactory>());
	 ...
	 */
	public MetadataResourceFactory getMetadataResourceFactory(File file) 
	...
	private MetadataResourceFactory getFactoryInstance(File file, String key) {
		...
		if ( result == null ) {
			...
			if ( result != null ) {
				// check if same class already configured, and if so
				// use that instance instead of a copy
	 ...
	 */
	protected MetadataResourceFactory findFactory(File file, String key) 
	throws MetadataConfigurationException {
		String[] searchPaths = disableSearchPath
			? new String[] {managerProperties}
			: managerProperties != null 
				? new String[] {managerProperties, CUSTOM_MANAGER_PROPERTIES, 
					DEFAULT_MANAGER_PROPERTIES}
				: new String[] {CUSTOM_MANAGER_PROPERTIES,  DEFAULT_MANAGER_PROPERTIES};
		...
		throw new MetadataConfigurationException(
				"Unable to locate suitable factory configuration. Searched in "
				+Arrays.toString(searchPaths));
	}
	
	private MetadataResourceFactory lookupConfiguration(String key, String propPath) {
		if ( propPath == null ) {
			throw new MetadataConfigurationException(
					"Manager properties not specified");
		}
		URL resource = getClass().getClassLoader().getResource(propPath);
		if ( resource == null ) {
			throw new MetadataConfigurationException(
					"Manager properties not found: " +propPath);
		}
		...
		String factoryClassName = properties.getProperty(
				FACTORY_PROPERTY_KEY_PREFIX+key);
		...
		try {
			...
			if ( !(factory instanceof MetadataResourceFactory) ) {
				throw new MetadataConfigurationException("Manager property [" 
						+FACTORY_PROPERTY_KEY_PREFIX+key 
						+"] factory class does not implement "
						+MetadataResourceFactory.class);
			}
