 ...
 * 
 * <p>
 * This class manages finding camera-specific {@link MetadataResource}
 ...
 * properties resource is loaded via the same ClassLoader that loaded this
 * class, i.e. <code>getClass().getClassLoader().getResource()</code> is used.
 * The properties file must have the format
 * </p>
 * 
 * <pre>
 ...
 * 
 * <p>
 * where <code>MAKE</code> is an EXIF camera make (spaces removed),
 ...
 * {@link MetadataResource}. The {@link MetadataResource} implementation must
 * provide a constructor that takes a single {@link Metadata} object as a
 * parameter, which will be invoked by this factory. The class will be loaded
 * via the same ClassLoader that loaded this class, i.e.
 * <code>getClass().getClassLoader().loadClass()</code> is used.
 * </p>
 * 
 * <p>
 * The <code>MODEL</code> may also be specified as a wildcard by using a
 * <code>*</code> value. In this case, if a specific model match is not found,
 * the wildcard value will be used.
 * </p>
 * 
 * <p>
...
public class EXIFMetadataResourceFactory implements MetadataResourceFactory {
	...
	private Map<String, Map<String, Class<? extends MetadataResource>>> makeModelMap = new LinkedHashMap<String, Map<String, Class<? extends MetadataResource>>>();
	 ...
	 * 
	 * <p>
	 * Adapted from Metadata Extractor (http://drewnoakes.com/code/exif/).
	 * </p>
	 * 
	...
	private void setupCameraMapping() {
		String[] searchPaths = new String[] { DEFAULT_CAMERA_PROPERTIES, CUSTOM_CAMERA_PROPERTIES };
	...
	private void setupCameraConfiguration(String propPath) {
		...
		try {
			...
			throw new MetadataConfigurationException(
					"Unable to load factory configuration properties [" + propPath + "]", e);
		}
		...
		for ( Object key : properties.keySet() ) {
			...
			if ( !propKey.startsWith(CAMERA_PROPERTY_KEY_PREFIX) )
				continue;
			int makeModelDelimIdx = propKey.indexOf('.', CAMERA_PROPERTY_KEY_PREFIX.length());
			if ( makeModelDelimIdx == -1 ) {
				log.warn("Ignoring bad camera property: " + propKey);
				continue;
			}
			String make = propKey.substring(CAMERA_PROPERTY_KEY_PREFIX.length(), makeModelDelimIdx)
					.toLowerCase();
			...
			if ( !makeModelMap.containsKey(make) ) {
				makeModelMap.put(make, new LinkedHashMap<String, Class<? extends MetadataResource>>());
			}
			Map<String, Class<? extends MetadataResource>> makeMap = makeModelMap.get(make);

			Class<? extends MetadataResource> modelClass = null;
			try {
				modelClass = getClass().getClassLoader().loadClass(properties.getProperty(propKey))
						.asSubclass(MetadataResource.class);
			} catch ( Exception e ) {
				throw new MetadataConfigurationException("Unable to instantiate MetadataResource class ["
						+ properties.getProperty(propKey) + "]", e);
			}

			if ( log.isDebugEnabled() ) {
				log.debug("Configured camera mapping for make [" + make + "]; model [" + model + "]: "
						+ modelClass.getName());
			}
	 ...
	 * 
	 * <p>
	 * If no make/model specific configuration can be found, a
	 * {@link EXIFJpegMetadataResource} instance will be returned.
	 * </p>
	 * 
	 * @param in
	 *        the input stream
	...
	private MetadataResource getJpegMetadataResource(InputStream in) {
		...
		if ( makeModelMap.containsKey(make) ) {
			...
			if ( makeExifClass != null ) {
				try {
					Constructor<? extends MetadataResource> constr = makeExifClass
							.getConstructor(Metadata.class);
	...
	private String getExifStringKey(Metadata exif, Class<? extends Directory> dirClass, int tagType) {
		Directory dir = exif.getFirstDirectoryOfType(dirClass);
